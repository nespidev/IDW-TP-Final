...
exports.getAllAlojamientos = async (req, res) => {
  try {
    ...
    const [rows] = await connection.query('SELECT * FROM Alojamiento');
    connection.release();
    res.json(rows);
  } catch (error) {
...
exports.createAlojamiento = async (req, res) => {
  try {
    ...
    const connection = await dbConnection.getConnection();
    const [results] = await connection.query('INSERT INTO Alojamiento SET ?', nuevoAlojamiento);
    connection.release();
...
exports.updateAlojamiento = async (req, res) => {
  try {
    ...
    const connection = await dbConnection.getConnection();
    await connection.query('UPDATE Alojamiento SET ? WHERE idAlojamiento = ?', [datosAActualizar, id]);
    connection.release();
...
exports.deleteAlojamiento = async (req, res) => {
  try {
    const { id } = req.params;
    const connection = await dbConnection.getConnection();
    await connection.query('DELETE FROM Alojamiento WHERE idAlojamiento = ?', [id]);
    connection.release();
